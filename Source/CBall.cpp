...
namespace game_framework {
	...
	bool CBall::HitEraser(Character* eraser)
	{
		// 檢測擦子所構成的矩形是否碰到球
	...
	bool CBall::HitRectangle(int tx1, int ty1, int tx2, int ty2)
	{
		...
		int x2 = x1 + golds.Width();	// 球的右下角x座標
		int y2 = y1 + golds.Height();	// 球的右下角y座標
									//
									// 檢測球的矩形與參數矩形是否有交集
									//
	...
	{
		golds.LoadBitmap(IDB_COIN_0, RGB(255, 255, 255));
	...
	{
		if (is_alive) {
			//golds.SetTopLeft(x + dx, y + dy);
			golds.SetTopLeft(tlx, tly);
			golds.ShowBitmap();
